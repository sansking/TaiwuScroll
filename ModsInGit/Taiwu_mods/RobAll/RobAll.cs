...
namespace RobAll
{
    ...
    public static class Main
    {
        ...
        static void OnGUI(UnityModManager.ModEntry modEntry)
        {
            ...
            {
                labelText = Rob();
                GEvent.OnEvent(eEvents.OnShowNotifyEvent, labelText);
            }
            GUILayout.Label(labelText);
            settings.robResource = GUILayout.Toggle(settings.robResource, "搜刮资源");
        ...
        static string Rob()
        {
            ...
            if (DateFile.instance == null || mianId <= 0) return "存档未载入";
            int victimCnt = 0;
            int itemCnt = 0;
            foreach (int actorId in DateFile.instance.actorFamilyDate)
            {
                ...
                if (settings.robResource)
                {
                    var resourceSize = DateFile.instance.ActorResource(actorId);
                    for (int i = 0; i < 6; i++)
                    {
                        UIDate.instance.ChangeTwoActorResource(actorId, mianId, i, resourceSize[i], true);
                        Main.Logger.Log($"{DateFile.instance.massageDate[7018][0].Split('|')[i]}</color>:{resourceSize[i]}");
        ...
        static void ChangeItem(int actorId, int itemId, int itemNumber = 1)
        {
            ...
            GameData.Characters.SetCharProperty(actorId, 210, (int.Parse(DateFile.instance.GetActorDate(actorId, 210, false)) + favorChange).ToString());//债
            DateFile.instance.ChangeFavor(actorId, -favorChange, false, false);
        }
        static bool Match(int itemId)
        {
            if (DateFile.instance.GetItemDate(itemId, 5, false) == "42") return false;
